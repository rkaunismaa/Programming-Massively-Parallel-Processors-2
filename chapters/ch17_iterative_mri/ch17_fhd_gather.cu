/*
 * ch17_fhd_gather.cu — Chapter 17: Iterative MRI Reconstruction
 * Fig 17.9 + Fig 17.10 + Fig 17.7: Gather approach with loop interchange
 *
 * Loop interchange transforms the scatter approach into gather:
 * outer loop = n (voxels), inner loop = m (k-space samples).
 * Each thread computes one voxel — no atomics needed.
 *
 * Build: nvcc -std=c++17 -arch=sm_61 -O2 -o ch17_fhd_gather ch17_fhd_gather.cu
 * Run:   ./ch17_fhd_gather
 */
...
// Each thread accumulates contributions from ALL k-space samples (m)
// Loop-interchanged: outer=n (voxels), inner=m (k-space samples)
// No atomics needed — each thread owns its output voxel
// ============================================================
#define FHD_THREADS_PER_BLOCK 256

__global__ void cmpFhD_gather(const float* __restrict__ kx, const float* __restrict__ ky,
    ...
    for (int m = 0; m < M; m++) {
        float expFhD = 2.0f * PI * (kx[m] * xn + ky[m] * yn + kz[m] * zn);
...
int main() {
    ...
    printf("=== Chapter 17: FHD Gather Kernel (Fig 17.10 + 17.7) ===\n");
    ...
    // Step 2: cmpFhD gather (no atomics needed)
    printf("\nLaunching cmpFhD gather kernel: %d blocks x %d threads\n", fhd_blocks, FHD_THREADS_PER_BLOCK);

    // Warm-up
    CHECK_CUDA(cudaMemset(rFhD_d, 0, szN));
    CHECK_CUDA(cudaMemset(iFhD_d, 0, szN));
    cmpFhD_gather<<<fhd_blocks, FHD_THREADS_PER_BLOCK>>>(kx_d, ky_d, kz_d,
