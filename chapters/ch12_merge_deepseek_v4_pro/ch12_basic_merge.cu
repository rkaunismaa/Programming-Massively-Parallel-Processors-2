 ...
 *  Chapter:   12 — Merge
 *  Figures:   12.2 (merge_sequential), 12.5 (co_rank), 12.9 (basic kernel)
 *  Hardware:  GTX 1050, sm_61 (Pascal)
 *
 *  Approach: Each thread gets an output segment of ~total/total_threads elements.
 *  Two co_rank calls determine the input subarrays from A and B that produce
 *  this thread's output segment. Then merge_sequential() merges those two
 *  subarrays into the output segment.
 *
 *  Key functions:
 *   - merge_sequential(A,m, B,n, C)      — Fig 12.2: two-finger sequential merge
 *   - co_rank(k, A,m, B,n)               — Fig 12.5: binary-search co-rank
 *   - merge_basic_kernel(A,m, B,n, C)    — Fig 12.9: one thread per output segment
 * =============================================================================
 */
...
__device__ __host__ int co_rank(
    int k,              // rank (output position) in the merged array C
    ...
    int i = (k < m) ? k : m;
    int j = k - i;      // invariant: i + j = k

    // Lower bounds: at least (k - n) elements must come from A
    int i_low = (k > n) ? (k - n) : 0;
    int j_low = (k > m) ? (k - m) : 0;
    ...
    while (active) {
        if (i > 0 && j < n && A[i - 1] > B[j]) {
            // i is too high — more A elements than necessary, shrink i
            int delta = (i - i_low + 1) >> 1;
            if (delta < 1) delta = 1;  // CRITICAL: prevent infinite loop
            j_low = j;   j += delta;
            i_low = i;   i -= delta;
        } else if (j > 0 && i < m && B[j - 1] > A[i]) {
            // j is too high — more B elements than necessary, shrink j
...
__global__ void merge_basic_kernel(
    ...
    // First co-rank: find input range start for this thread
    int i_curr = co_rank(k_curr, A, m, B, n);
    int j_curr = k_curr - i_curr;

    // Second co-rank: find input range end (= start of next thread's range)
    ...
    // Sequential merge on this thread's subarrays
...
int main() {
    ...
    printf("\n===== ch12_basic_merge — Basic Parallel Merge =====\n");
    printf("Each thread merges its own output segment (~elems_per_thread = total/total_threads)\n");
    ...
    for (int t = 0; t < num_tests; t++) {
        ...
        for (int ratio = 0; ratio < 3; ratio++) {
            int m, n;
            if (ratio == 0) {
                m = total / 5;               // A is small (20% of total)
                n = total - m;
            } else if (ratio == 1) {
                m = total / 2;               // A ≈ B (50%)
                n = total - m;
            } else {
                m = 4 * total / 5;           // A is large (80%)
                n = total - m;
            }
            ...
            // Launch config: ~8 elements per thread on average
            int threads = 256;
            ...
            float throughput = (total * sizeof(int) * 3.0f) / (ms * 1e6f);  // GB/s
