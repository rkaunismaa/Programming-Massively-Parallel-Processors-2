 ...
 * Thread Coarsening — Interleaved Partitioning (Fig 9.14)
 *
 * Each thread processes MULTIPLE input elements, but elements are
 * INTERLEAVED across threads — in the first iteration thread 0 processes
 * element 0, thread 1 processes element 1, ..., then in the next iteration
 * all threads advance by blockDim.x * gridDim.x.
 *
 * This ensures CONSECUTIVE threads access CONSECUTIVE memory addresses,
 * enabling full memory coalescing — the key advantage over contiguous
 * partitioning.
 *
...
/*
 * Coarsened histogram kernel — interleaved partitioning (Fig 9.14).
 *
 * Each thread iterates with stride = blockDim.x * gridDim.x:
 *   for i = tid; i < length; i += stride
 *
 * This makes consecutive GPU threads access consecutive addresses.
 *
 ...
 */
__global__ void coarsened_interleaved_histogram_kernel(
    ...
    int length)
{
    ...
    // Each thread processes INTERLEAVED elements
    // Stride = total number of threads across all blocks
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    // Process CFACTOR elements with coalesced access
...
int main()
{
    ...
    std::cout << "=========================================================\n";
    std::cout << "Chapter 9: Coarsened Histogram (Fig 9.14)\n";
    std::cout << "          Interleaved Partitioning\n";
    std::cout << "=========================================================\n";
    ...
    // With interleaved: total threads = N/CFACTOR (approx)
    // stride = blockDim.x * gridDim.x, so gridDim.x = N / (blockDim.x * CFACTOR)
    int block_size = 256;
    ...
    std::cout << "Stride:     " << total_threads << " (total threads)\n\n";

    // Warm-up
    coarsened_interleaved_histogram_kernel<<<grid_size, block_size>>>(d_data, d_histo, N);
