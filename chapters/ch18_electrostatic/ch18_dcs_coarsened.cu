/*
 * ch18_dcs_coarsened.cu — Direct Coulomb Summation: Thread Coarsening (Fig 18.8)
 *
 * From PMPP 4th Ed, Chapter 18: Electrostatic Potential Map
 * Fig 18.8: Thread coarsening with COARSEN_FACTOR=4.
 *
 * Each thread computes energy for 4 CONSECUTIVE grid points in the x-dimension.
 * This enables:
 *   - Reuse of dy, dz, dy²+dz², and charge across all 4 points
 *   - Reuse of dx offset pattern (dx0, dx0+gridspacing, dx0+2*gridspacing, ...)
 *
 * Reduction vs Fig 18.6: 16 const mem accesses → 4, 48 FLOPs → 24 FLOPs
 *
 * Build: nvcc -std=c++17 -arch=sm_61 -O2 -o ch18_dcs_coarsened ch18_dcs_coarsened.cu
 * Run:   ./ch18_dcs_coarsened
 */
...
/*
 * Thread-coarsened gather kernel (Fig 18.8). COARSEN_FACTOR=4.
 * Each thread handles COARSEN_FACTOR consecutive grid points in x,
 * reusing dy, dz, dy²+dz², and charge for all 4.
 * Grid shrunk by COARSEN_FACTOR in x-dimension.
 *
 * Writes are to consecutive memory locations [i..i+3] (uncoalesced — fixed in Fig 18.10).
 */
__global__ void dcs_coarsened_kernel(float *energygrid, dim3 grid, float gridspacing,
                                      float z, int numatoms_in_chunk)
{
    ...
    for (int n = 0; n < atomarrdim; n += 4) {
        ...
        float dx2 = dx0 + 2.0f * gridspacing;
        float dx3 = dx0 + 3.0f * gridspacing;
    ...
    energygrid[base + 1] += energy1;
    energygrid[base + 2] += energy2;
    energygrid[base + 3] += energy3;
}

// CPU reference (same as gather kernel — identical computation, different organization)
...
int main()
{
    ...
    // Grid shrunk by COARSEN_FACTOR in x — ceiling division
    ...
    dcs_coarsened_kernel<<<grid_config, block_dim>>>(d_energygrid, grid_dim,
    ...
    printf("ch18_dcs_coarsened | GRID %dx%d | ATOMS %d | COARSE %d | %s | %.3f ms\n",
