/*
 * =============================================================================
 *  ch12_tiled_merge.cu — Tiled Parallel Merge Kernel
 * =============================================================================
 ...
 *
 *  Improves on the basic merge kernel by:
 *    1. Block-level co-rank: one thread per block calls co_rank on global memory
 *       to find block-level A and B input ranges. This reduces # of global binary
 *       searches from (#threads) to (#blocks).
 *    2. Cooperative tiled loading: all threads in a block load A and B tiles into
 *       shared memory in coalesced patterns.
 *    3. Thread-level co-rank on SHARED memory: individual threads call co_rank on
 *       the in-shared-memory tiles to find their individual input ranges.
 *    - Each thread then calls merge_sequential on its shared-memory subarrays.
 *    - The while-loop processes tile_size output elements per iteration.
 *
 *  Limitation: Only ~50% of loaded data is consumed per iteration (the rest is
 *  reloaded next iteration). The circular buffer kernel (ch12_circular_buffer_merge.cu)
 *  addresses this.
 * =============================================================================
 */
...
/* -------------------------------------------------------------------------- */
/*  Tiled Merge Kernel                                                        */
/*  Fig 12.11 — Part 1: Block-level output/input subarray identification       */
/*  Fig 12.12 — Part 2: Loading A and B tiles into shared memory               */
/*  Fig 12.13 — Part 3: Thread-level merge from shared memory                  */
/* -------------------------------------------------------------------------- */
...
__global__ void merge_tiled_kernel(
    ...
    // Each block is responsible for a block-level output subarray
    ...
    // Iterative tile processing (Fig 12.12 + 12.13)
    int A_consumed = 0;
    int B_consumed = 0;
    int C_completed = 0;

    while (C_completed < C_length) {
        // How many output elements to produce this iteration
        int tile_out = min(TILE_SIZE, C_length - C_completed);

        // ---- Part 2 (Fig 12.12): Load A tile into shared memory ----
        ...
        // ---- Part 3 (Fig 12.13): Thread-level merge from shared memory ----
        ...
        // Update accumulated consumption counters
        // total_A_used: number of A elements in first tile_out outputs
        if (tid == 0) {
...
int main() {
    ...
    for (int t = 0; t < num_tests; t++) {
        ...
        for (int ratio = 0; ratio < 3; ratio++) {
            ...
            // Block count: each block handles a segment of the output
            int threads = 128;
            int blocks = 16;
